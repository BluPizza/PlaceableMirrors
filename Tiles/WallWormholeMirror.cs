...
{
	public class WallWormholeMirror : ModTile
	{
		public override void SetStaticDefaults()
		{
			...
			name.SetDefault("{$Mods.PlaceableMirrors.Tiles.WormholeMirror}");
			AddMapEntry(new Color(37, 201, 252), name);
		...
		public override void DrawEffects(int x, int y, SpriteBatch sb, ref TileDrawInfo drawData)
		{
			...
			{
				Dust dust = Dust.NewDustDirect(new Vector2(x * 16, y * 16), 16, 16, DustID.DungeonSpirit);
				dust.noGravity = true;
			}
		}
		...
		{
			return ModContent.GetInstance<PlaceableMirrorsClientConfig>().EnableSmartInteractWormholeMirror;
		}

		public override void KillMultiTile(int x, int y, int frameX, int frameY)
		{
			Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 48, 32, ModContent.ItemType<Items.WallWormholeMirror>());
		}
